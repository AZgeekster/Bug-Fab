#!/usr/bin/env bash
#
# One-command cross-stack conformance check for the Bug-Fab Swift (Vapor) adapter.
#
# Boots the `BugFabExample` target in a swift:6.0 container, runs the Python
...
echo "--- boot adapter ---"
# `up -d adapter` starts only the Vapor server. The runner waits for the
...
echo "--- wait for :8080 to respond (up to 300s — first run compiles the package, ~5-10 min) ---"
...
while [ $SECONDS -lt $deadline ]; do
  state="$(docker inspect --format='{{.State.Health.Status}}' bug-fab-vapor-conformance-adapter 2>/dev/null || echo "missing")"
  case "$state" in
    healthy)
      healthy=1
      break
...
if [ "$healthy" -ne 1 ]; then
  ...
  docker logs bug-fab-vapor-conformance-adapter 2>&1 | head -80 >&2 || true
  exit 2
fi

echo "--- adapter is up — first 5 lines of server logs ---"
docker logs bug-fab-vapor-conformance-adapter 2>&1 | head -5 || true
echo ""

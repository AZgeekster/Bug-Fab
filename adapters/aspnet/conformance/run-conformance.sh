#!/usr/bin/env bash
#
# One-command cross-stack conformance check for the Bug-Fab ASP.NET Core adapter.
#
# Boots `examples/MinimalApi` in a dotnet/sdk:8.0 container, runs the Python
...
echo "--- boot adapter ---"
# `up -d adapter` starts only the ASP.NET server. The runner waits for the
...
echo "--- wait for :8080 to respond (up to 300s — first run restores NuGet packages) ---"
...
while [ $SECONDS -lt $deadline ]; do
  state="$(docker inspect --format='{{.State.Health.Status}}' bug-fab-aspnet-conformance-adapter 2>/dev/null || echo "missing")"
  case "$state" in
    healthy)
      healthy=1
      break
...
if [ "$healthy" -ne 1 ]; then
  ...
  docker logs bug-fab-aspnet-conformance-adapter 2>&1 | head -80 >&2 || true
  exit 2
fi

echo "--- adapter is up — first 5 lines of server logs ---"
docker logs bug-fab-aspnet-conformance-adapter 2>&1 | head -5 || true
echo ""

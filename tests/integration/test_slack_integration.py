...
def _hermetic_slack_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Strip pre-existing ``BUG_FAB_SLACK_*`` vars so ``from_env`` tests are hermetic.

    The tests used to ``delenv`` only the variables they named and failed
    spuriously on machines exporting other ``BUG_FAB_SLACK_*`` values.
    """
    clear_env_prefix(monkeypatch, "BUG_FAB_SLACK_")
